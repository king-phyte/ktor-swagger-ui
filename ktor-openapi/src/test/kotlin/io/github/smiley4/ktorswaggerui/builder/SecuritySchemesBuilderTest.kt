...
import io.github.smiley4.ktoropenapi.dsl.config.OpenApiSecurityScheme
...
class SecuritySchemesBuilderTest : StringSpec({
    ...
    companion object {
        ...
        private fun buildSecuritySchemeObjects(builders: Map<String, io.github.smiley4.ktoropenapi.dsl.config.OpenApiSecurityScheme.() -> Unit>): Map<String, SecurityScheme> {
            return SecuritySchemesBuilder(
                oAuthFlowsBuilder = OAuthFlowsBuilder()
            ).build(builders.map { (name, entry) -> io.github.smiley4.ktoropenapi.dsl.config.OpenApiSecurityScheme(name).apply(entry).build(SecuritySchemeData.DEFAULT) })
        }
